...
13. Interactive minimum:
   - Each lesson must include 3 required qualifying interactive elements (+ optional 4th+ when instructionally valuable).
   - Required interaction blueprint: tabs, accordion, checkpoint prompt (non-graded).
   - Optional 4th+ interactions are allowed only when instructionally valuable for that lesson and may follow candidates in `docs/future-upgrades.md`.
...
- Lesson must include 3 required interactions: tabs, accordion, checkpoint prompt (non-graded).
- Optional 4th+ interaction is permitted only when it adds clear instructional value.
